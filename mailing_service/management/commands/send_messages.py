...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        mailings = Mailing.objects.filter(status="Created")  # Отправляем только не запущенные рассылки
        for mailing in mailings:
            ...
            mailing_attempt = MailingAttempt.objects.create(
                ...
                server_response="\n".join(errors) or f"Отправлено {success_count} из {len(recipients)}",
                mailing=mailing
            )

            # Обновляем статус рассылки
            mailing.status = "Completed" if success_count == len(recipients) else "Running"
